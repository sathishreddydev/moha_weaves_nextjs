      ...
      {success ? (
        <div className="flex items-center gap-2 py-3 text-green-700">
          ...
          <div>
            ...
            <p className="text-[10px] text-green-600">Thank you for your feedback.</p>
          </div>
        </div>
      ) : (
        <div className="space-y-3">
          ...
          {/* Title */}
          <div className="space-y-1">
            <Label htmlFor={`review-title-${item.id}`} className="text-xs text-slate-600">
              Title{" "}
          ...
          {/* Comment */}
          <div className="space-y-1">
            <Label htmlFor={`review-comment-${item.id}`} className="text-xs text-slate-600">
              Your Review *
            </Label>
            <Textarea
...
export function OrderItem({
  ...
  const [localReview, setLocalReview] = useState<OrderItemType["reviewInfo"] | null>(null);
  ...
  const showReviewButton = isDelivered && !isInReturn && !isInExchange && !alreadyReviewed && !reviewOpen;
    ...
    <Card className="p-4 hover:border-slate-300 transition-colors bg-white">
      ...
      <div className="flex flex-wrap items-center gap-3 pt-3 mt-3 border-t border-slate-100">
        ...
        <div className="flex items-center gap-2 ml-auto">
          {/* Review button — opens inline panel */}
          {showReviewButton && (
            <button
              type="button"
    ...
    <div className="mt-3 pt-3 border-t border-yellow-100">
      ...
      {open && (
        <div className="mt-2 space-y-2">
          ...
          {/* Title */}
          {review.title && (
            <p className="text-xs font-semibold text-gray-800">{review.title}</p>
          )}
          ...
          {images.length > 1 && (
            <>
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  setLightboxIndex((i) => (i === 0 ? images.length - 1 : (i ?? 0) - 1));
                }}
              ...
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  setLightboxIndex((i) => (i === images.length - 1 ? 0 : (i ?? 0) + 1));
                }}
...
function getReturnStep(status: string): "requested" | "in_progress" | "done" | "rejected" {
      ...
      {steps.map((s, idx) => {
        ...
        return (
          <div key={s.key} className="flex items-center gap-1 flex-1 min-w-0">
            <div className="flex flex-col items-center gap-0.5 flex-shrink-0">
              <div
                className={`w-4 h-4 rounded-full flex items-center justify-center ${
                  done || active ? "bg-orange-500 text-white" : "bg-slate-200 text-slate-400"
                }`}
              >
                {done ? (
            ...
            {idx < steps.length - 1 && (
              <div className={`h-px flex-1 mx-1 ${done ? "bg-orange-400" : "bg-slate-200"}`} />
            )}
          </div>
        );
      })}
...
function RefundDetail({ refund, refundAmount }: { refund: RefundInfo; refundAmount?: string }) {
    ...
    return (
      <div className="flex items-start gap-2 text-[10px] text-red-600 bg-red-50 rounded-lg px-3 py-2">
        ...
        <div>
          ...
          {refund.failureReason && <p className="text-red-500 mt-0.5">{refund.failureReason}</p>}
            ...
            return (
              <div key={s.key} className="flex items-center gap-1 flex-1 min-w-0">
                <div className="flex flex-col items-center gap-0.5 flex-shrink-0">
                  <div
                    className={`w-4 h-4 rounded-full flex items-center justify-center ${
                      done || active ? "bg-blue-500 text-white" : "bg-slate-200 text-slate-400"
                    }`}
                  >
                    {done ? (
                      <CheckCircle className="w-3 h-3" />
                ...
                {idx < steps.length - 1 && (
                  <div className={`h-px flex-1 mx-1 ${done ? "bg-blue-400" : "bg-slate-200"}`} />
                )}
              </div>
            );
          })}
